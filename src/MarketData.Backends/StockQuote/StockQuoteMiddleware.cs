...
namespace MarketData.Backends.StockQuote.Middlewares
{
    public class StockQuoteMiddleware
    {
        ...
        public StockQuoteMiddleware(RequestDelegate next, ILogger<StockQuoteMiddleware> logger, StockQuoteProvider stockQuoteProvider)
        {
        ...
        {
            var stockQuoteStream = _stockQuoteProvider.GetEventStream();
            var webSocket = await context.WebSockets.AcceptWebSocketAsync();
            var webSocketClosedStream = StartPolling(webSocket).ToObservable();

            await stockQuoteStream.Select(quote => (webSocket, quote))
