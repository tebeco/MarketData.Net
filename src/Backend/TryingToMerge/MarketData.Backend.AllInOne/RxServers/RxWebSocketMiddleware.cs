...
namespace MarketData.Backend.AllInOne.RxServers
{
    public class RxWebSocketMiddleware<T, U> where T : IStreamProvider<U> where U : IProduceJson
    {
        ...
        private readonly T _streamProvider;

        public RxWebSocketMiddleware(RequestDelegate next, ILogger<RxWebSocketMiddleware<T, U>> logger, T streamProvider)
        {
        ...
        {
            var forexStream = _streamProvider.GetEventStream();
            var webSocket = await context.WebSockets.AcceptWebSocketAsync();
            var webSocketClosedStream = StartPolling(webSocket).ToObservable();

            await forexStream.Select(quote => Tuple.Create(webSocket, quote))
                //.CombineLatest(socketObservable, forexStream, (socket, quote) => (socket, quote))
